...
def generate_synthesis(raw: dict, dims_scored: dict, panel: dict, agent_analysis: dict | None = None) -> dict:
    ...
    return {
        ...
        # v3.9.1 · 用 .get() 而非 []，兼容 panel_skipped（空 panel）时 bull/bear 为空 dict 的情况
        "debate": {
            "bull": {"investor_id": bull.get("investor_id"), "name": bull.get("name"), "group": bull.get("group")},
            "bear": {"investor_id": bear.get("investor_id"), "name": bear.get("name"), "group": bear.get("group")},
            "rounds": rounds,
            "punchline": punchline,
        },
        "great_divide": {
            "bull_avatar": bull.get("investor_id"),
            "bear_avatar": bear.get("investor_id"),
            "bull_score": bull.get("score"),
            "bear_score": bear.get("score"),
            "bull_signal": bull.get("signal"),
            "bear_signal": bear.get("signal"),
            "punchline": punchline,
        },
        "risks": risks,
